...
This is a source-bounded repository audit summary, not a new literature search.

## RW01: Kivlichan et al., Hamming-weight phasing
- **DOI**: arXiv:1902.10673v4
- **Type**: baseline
- **Delta**: The local implementation retains staged compressors but uses reversed unitary Toffolis rather than the source's cheaper measured cleanup.
- **Claims affected**: [C02, C03]
- **Adopted elements**: Staged HWP arithmetic and batching.

## RW02: Amy, Maslov, Mosca, Roetteler; exact Toffoli
- **DOI**: arXiv:1206.0758v4
- **Type**: imports
- **Delta**: Shared exact primitive lowering; complete emitted circuits are rescheduled instead of adding isolated primitive depths.
- **Claims affected**: [C02, C03]
- **Adopted elements**: Exact seven-T, three-layer Toffoli template.

## RW03: T-par and relational phase folding
- **DOI**: Not resolved in this initialization; see the repository matrix for pinned Feynman sources.
- **Type**: baseline
- **Delta**: Earlier runs use released T-par/phasefold; current relational passes remain unbuilt. The local scratch action is a limited reproduction, not full T-par.
- **Claims affected**: [C01, C03]
- **Adopted elements**: Optional exact verified backend integration.

## RW04: Kan-Symons catalytic HWP
- **DOI**: arXiv:2411.02160v2
- **Type**: bounds
- **Delta**: Catalyst preparation, repeated-use accounting, restoration, and measured cleanup remain outside the emitted unitary competitors.
- **Claims affected**: [C02, C03]
- **Adopted elements**: Audit boundary only; no eligible catalytic adapter.

## Complete Repository Citation and Adapter Matrix

The following existing matrix is preserved verbatim to retain its full footprint, including synthesis, verification, alternative backends, and availability limitations. Its original audit date applies.
...
| Amy-Maslov-Mosca-Roetteler, arXiv:1206.0758v4, Figure 13 | Exact ancilla-free Toffoli with seven T gates in three T layers | Shared local primitive template | Dense eight-dimensional CCX comparison checks action and global phase; the complete emitted circuit is rescheduled after substitution |
...
| PyZX `0.9.0`, Apache-2.0 | Fully emitted Clifford+T stream | Python adapter exposes basic cancellation, `full_optimize`, and full ZX reduction/extraction | Every accepted output passes source/result ZX equivalence; an explicit scalar correction preserves the repository's global-phase convention |
| Qualtran `HammingWeightCompute`, revision `d5faa99db850001d440d0cc4e05b7b20b249a652` | Out-of-place Hamming weight using temporary ANDs | Apache-2.0 source inspected; no code copied or runtime dependency added | Corroborates the staged compressor order and `n-popcount(n)` count; register layout differs from the local direct-input and copied-parity unitary adaptations |
| Kan-Symons, arXiv:2411.02160v2, "Space-time trade-off in Hamming-weight phasing" and Supplementary Fig. 3 | Equal-angle HWP; generalized phase gradient; angle-specific reusable catalyst; measured cleanup | Paper formulas only; the former local estimate was removed from active code | Audit records catalyst preparation, restoration, extra `floor(log2 M)+1` Toffolis, and repeated-use requirements; no channel/catalyst validation exists |
